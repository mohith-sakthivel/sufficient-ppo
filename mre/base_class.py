...
class BaseAlgorithm(ABC):
    ...
    :param pol_kwargs: Additional arguments to be passed to the policy on creation
    :param tb_log: the log location for tensorboard (if None, no logging)
    ...
    def __init__(
        ...
        self.pol_kwargs = {} if pol_kwargs is None else pol_kwargs
    ...
    def _update_remain_progress(self, num_timesteps: int, total_timesteps: int) -> None:
        """
        ...
        """
        self._remain_progress = 1.0 - float(num_timesteps) / float(total_timesteps)

    def _update_lr(self, optimizers: Union[List[th.optim.Optimizer], th.optim.Optimizer]) -> None:
        """
        ...
        logger.record("train/lr", self.lr_schedule(self._remain_progress))

        if not isinstance(optimizers, list):
            optimizers = [optimizers]
        for optimizer in optimizers:
            update_learning_rate(optimizer, self.lr_schedule(self._remain_progress))
    ...
    def _setup_learn(
        ...
        utils.configure_logger(self.verbose, self.tb_log, tb_log_name, reset_num_timesteps)
    ...
    def load(
        ...
        **kwargs,
        ...
        if "pol_kwargs" in data:
            if "device" in data["pol_kwargs"]:
                del data["pol_kwargs"]["device"]

        if "pol_kwargs" in kwargs and kwargs["pol_kwargs"] != data["pol_kwargs"]:
            raise ValueError(
                f"The specified policy kwargs do not equal the stored policy kwargs."
                f"Stored kwargs: {data['pol_kwargs']}, specified kwargs: {kwargs['pol_kwargs']}"
            )
